...
public abstract class GroveRgbLcd implements AutoCloseable {
  ...
  public static final HashMap<Character, Integer> specialChars = new HashMap<Character, Integer>() {{ put('μ', 0xE4); put('°', 0xDF); }};
    ...
    execTEXT((io) -> {
      ...
      for (char c : text.toCharArray()) {
        ...
        if(specialChars.containsKey(c)) {
          io.write(LCD_WRITECHAR, specialChars.get(c)); // Write character
        } else {
